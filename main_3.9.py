...
def reconstruct_path(predecessors, start, goal):
    ...
    while current != start:
        ...
        path.append(current)
    path.reverse()  # 逆序以得到从起点到目标点的路径
    path_length = len(path) - 1  # 路径长度为节点数量减一
...
def dijkstra_to_multiple_goals_with_path_and_length(grid, start, goals):
    ...
    paths_and_lengths = {goal: reconstruct_path(predecessors, start, goal) for goal in found_goals}
    ...
    def move(self):
        ...
        if self.goods == 1:  ###一开始货物很少 我们让机器人直接去码头
            ...
            if self.route_index == 0:
                paths_and_lengths = dijkstra_to_multiple_goals_with_path_and_length(mapp, (self.x,self.y), goal_berth)
                shortest_length = float('inf')
                shortest_path_goal = None
                shortest_path = []

                for goal, (path, length) in paths_and_lengths.items():
                    if length < shortest_length:
                        shortest_length = length
                        shortest_path_goal = goal
                        shortest_path = path

                # self.mbx=shortest_path_goal[0][0]
                # self.mby=shortest_path_goal[0][1]

                self.route = jiexi_path(shortest_path)

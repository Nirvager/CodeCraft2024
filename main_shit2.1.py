...
def find_bestgoods(robot_pos, goods, current_frame):
    # 过滤掉在机器人到达前会消失的商品
    available_goods = [good for good in goods if good[1] + 1000 > current_frame + path[robot_pos][good[0]][1]]

    if len(available_goods) > 0:
        # 在剩余的商品中找到单位距离价值最大的商品
        max_value_goods = max(available_goods, key=lambda good: good[2] / path[robot_pos][good[0]][1])
    ...
    def search_goods_after_dijie(self,dijiecnt):
        ...
        if self.goods_route == []:
            ...
            # 离港口点很远 直接迪杰斯特拉 主要是为了应对 交接时段以及物品被抢占
                ...
                if distance(nearest_goods[0], (self.x, self.y)) >= 50 or dijiecnt >=1:
                    self.mbgx=0
                    self.mbgy=0
                    # 这个地方可以修改
...
class Boat:
    ...
    def __init__(self, num=0, pos=0, status=0):
        self.num = num
        self.pos = pos
        self.status = 0 #status 0: 虚拟点 1：港口 2: 途中
        self.start_zhen = 0
        self.arrive_zhen = 0
        self.goods_num = 0
        self.berth = -1

    #TODO 不太清楚虚拟点工作原理，最好在时间检测的同时加上位置检测，防止出现在虚拟点排队导致出发和到达时间差不为500帧时g掉的情况
    def go_and_come(self, berth):
        if zhen == 14490:
            print("go", self.num)
        elif zhen < 14490:
            if self.status == 2:
                if self.arrive_zhen == zhen:
                    if self.berth == -1:
                        self.status = 0
                    else:
                        self.status = 1
            if self.status == 0:
                no_boat_berths = [berth for berth in berth if not berth.is_have_boat]
                sorted_no_boat_berths = sorted(no_boat_berths, key=lambda berth: berth.goods_num, reverse=True)
                # 获取货物数量最多的港口
                max_goods_berth = sorted_no_boat_berths[0]
                self.start_zhen = zhen
                self.arrive_zhen = zhen + 500 #可能是499/501，有bug的话可以改一下试试
                self.status = 2
                self.berth = max_goods_berth.berth_id
                berth[self.berth].is_have_boat = True
                print("ship", self.num, max_goods_berth.berth_id)
            if self.status == 1:
                self.goods_num = (zhen - self.arrive_zhen) * berth[self.berth].loading_speed
                if berth[self.berth].goods_num == 0 or self.goods_num >= 70:
                    self.status = 2
                    self.start_zhen = zhen
                    self.arrive_zhen = zhen + 500 #可能是499/501，有bug的话可以改一下试试
                    berth[self.berth].is_have_boat = False
                    self.berth = -1
                    self.goods_num = 0
                    print("go", self.num)

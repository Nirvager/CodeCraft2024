...
class Robot:
    ...
    def search_goods(self):
       # direction = wander(self.x,self.y)   #用来保底的一个操作 不知道为什么有时候会爆
        ...
        if self.goods_route == [] or self.mbgx==0 and self.mbgy == 0:
            ...
            if distance(nearest_goods[0],(self.x, self.y)) >= 30:
        ...
        if distance((self.x, self.y), (self.mbgx, self.mbgy)) < 10:

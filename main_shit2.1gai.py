...
def filter_points(mapp, special_point, max_distance=100):
    """过滤出距离 special_points 小于 max_distance 的点"""
    filtered_points = []
    for point in np.argwhere(mapp == 1):
        point_tuple = tuple(point)
        # 检查该点与 special_points 中每个点的距离
        if distance(point_tuple, special_point) < max_distance:
            filtered_points.append(point_tuple)
    return filtered_points


def create_route_memory():
    for i in range(0, 10, 2):
        goals = filter_points(mapp,(berth[i].x,berth[i].y))
        path[(berth[i].x,berth[i].y)]= dijkstra_to_multiple_goals_with_path_and_length(mapp, special_points[i], goals)
        goals = filter_points(mapp, (berth[i+1].x, berth[i+1].y))
        path[(berth[i+1].x,berth[i+1].y)] = dijkstra_to_multiple_goals_with_path_and_length(mapp, special_points[i + 1], goals)
    # for i in range(0, 4, 2):
    #     goals = filter_points(mapp, (special_points[i][0],special_points[i][1]))
    #     path[(special_points[i][0],special_points[i][1])]= dijkstra_to_multiple_goals_with_path_and_length(mapp, special_points[i], goals)
    #     goals = filter_points(mapp, (special_points[i+1][0], special_points[i+1][1]))
...
def find_nearestberth(robot_pos, berth):
    ...
    nearest_berth = min(berth, key=lambda berth_pos: distance2(robot_pos, (berth_pos.x, berth_pos.y)))
...
class Robot:
    ...
    def search_goods_after_dijie(self,dijiecnt):
        ...
        if tmp_good != None:
            if distance(tmp_good[0], (self.x, self.y)) < distance((self.x, self.y), (self.mbgx, self.mbgy)):
                ...
                #     goods.insert(0,((self.mbgx,self.mbgy),self.mbg_zhen,self.mbg_val))
        ...
        if self.goods_route == []:
            ...
            # 到了港口点 直接找路径
                ...
                if (nearest_goods[0]) in path[(self.x, self.y)]:
                    tmp_path = path[(self.x, self.y)][nearest_goods[0]][0]
                    self.goods_route=tmp_path[::-1]
            ...
            # 离港口点很远 直接迪杰斯特拉 主要是为了应对 交接时段以及物品被抢占
                ...
                if distance(nearest_goods[0], (self.x, self.y)) >= 20 or dijiecnt >=1:
                    self.mbgx=0
                    self.mbgy=0
                    direction = get_best_direction((self.x, self.y), nearest_goods[0], self.move_history)
                    self.move_history.append(((self.x, self.y), direction))
    ...
    def search_berth_after_dijie(self,i):
        ...
        if self.route == []:
            ...
            if (self.x,self.y) in path[(nearest_berth.x, nearest_berth.y)]:
...
def Init(mapp,t1):
    ...
    for i in range(berth_num):
        ...
        sys.stderr.write(str(berth[id].x)+" "+str(berth[id].y)+"\n")
...
def int_is_robot_at_any_port(robot, berth):
    t = 1
    for port in berth:
        if port.x == robot.x and port.y == robot.y:
            return t
        t += 1
    return 0
...
if __name__ == "__main__":
    ...
    for zhen in range(1, 15001):
        ...
        if t1.is_alive():  #当迪杰斯特拉整体没有算完的时候
            ...
            for i in range(robot_num):
                if check_if_there_is_same_next_pos_before(i) == True:    #这里有一个传奇bug 三个一串撞 解决方案是把楼下的复制过来 但好像会变得很慢
                    # 停下来
                    if robot[i].route:
                        robot[i].route_berth_index -= 1
                    if robot[i].goods_route:
        ...
        for i in range(5):
            ...
            # 首次发船
            if zhen == 1000:
                sorted_berth_goods_num = sorted(berth, key=lambda berth: berth.goods_num,
                                                reverse=True)  # 对100帧时的泊位进行货物数的降序排列

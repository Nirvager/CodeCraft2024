...
class Robot:
    ...
    def search_goods_after_dijie(self,dijiecnt):
        ...
        if self.goods_route == []:
            ...
            # 没到港口点 但是非常近
            if (self.x, self.y) != (nearest_berth.x, nearest_berth.y) and distance2((self.x, self.y), (nearest_berth.x, nearest_berth.y)) <= 8:
...
if __name__ == "__main__":
        ...
        if t1.is_alive():  #当迪杰斯特拉整体没有算完的时候
            ...
            for i in range(robot_num):
                ...
                if check_if_there_is_a_duizhuangcrash_before(i) == True:
                    result =check_another_possible_direction(robot[i].x,robot[i].y,direction_list[i],list_next_pos,i)
                    if len(result) == 2:
                        direction_list[i], list_next_pos[i]=result
                    else:
                        direction_list[i]=-2
                        list_next_pos[i]=list_current_pos[i]
                    if robot[i].route:
    ...
    ##############################优化点: 可采用cz同志的停留帧数x装载速度求出最终货物数 不需要每一次循环都执行本条语句############################################
                                ...
                                berth[boat[i].pos].goods_num-=boat[i].num
